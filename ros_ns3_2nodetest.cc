...
void posecallback(const turtlesim::Pose::ConstPtr& msg){
        double pos_x = msg->x;
        double pos_y = msg->y;
        double pos_theta = msg->theta;
        double vel = msg->linear_velocity;
        double ang = msg->angular_velocity;
        
        double pose_arr[5] = {pos_x, pos_y, pos_theta, vel, ang};
        for(int i=0;i<5;i++){
                        //pose_str.append(to_string(pose_arr[i]));
...
int x= 0;
 ...
 main (int argc, char *argv[])
 {
   ...
   std::string macServer ("e8:4e:06:32:17:a6");

   ros::init(argc,argv,"NS3_pub");
   ros::NodeHandle n;
   ros::Subscriber sub = n.subscribe("coords",1000,posecallback);
